...
def get_loader(hp, config, shuffle=True):
    ...
    def collate_fn(batch):
        ...
        missing = hp.model_name
        if 'text' in missing:
            bert_sentences = torch.zeros_like(bert_sentences)
            bert_sentence_types = torch.zeros_like(bert_sentence_types)
            bert_sentence_att_mask = torch.zeros_like(bert_sentence_att_mask)

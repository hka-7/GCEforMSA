...
class GCE(nn.Module):
    def __init__(self, hp):
        ...
        self.conv_lv = conv_SubNet(self.dim_l, self.dim_v, self.dim_conv_out)
        self.conv_va = conv_SubNet(self.dim_v, self.dim_a, self.dim_conv_out)
        self.conv_la = conv_SubNet(self.dim_l, self.dim_a, self.dim_conv_out)
        # relation filter
    ...
    def forward(self, sentences, visual, acoustic, v_len, a_len, bert_sent, bert_sent_type, bert_sent_mask, y=None):
        ...
        # fusion
        massagehub_lv = self.conv_lv(text, visual).squeeze() # 128, 64
        massagehub_la = self.conv_la(text, acoustic).squeeze() # 128, 64
        massagehub_va = self.conv_va(visual, acoustic).squeeze() # 128, 64
        ...
        fusion_loss_repr, error_score, distilled_samples = self.fusion_modal_relation_trans.relation_transfer_3modality(fusion_detach_logits, gt, fusion_reprs) # fusion_loss_repr: torch.Size([]); error_score: torch.Size([3]); distilled_samples:list
        ...
        return [preds, self.ori_prj_list, self.fusion_prj_list], [loss_contrastive, loss_relation_filter]

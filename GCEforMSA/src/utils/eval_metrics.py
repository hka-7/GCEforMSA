...
def eval_mosei_senti(results, truths, exclude_zero=False):
    ...
    logger_info = f"MAE = {mae}\n"\
                    f"Correlation Coefficient = {corr}\n"\
                    f"mult_acc_7 = {mult_a7}\n"\
                    f"mult_acc_5 = {mult_a5}\n"\
                    f"F1 score all/non0: {np.round(f_score, 4)}/{np.round(f_score_non0, 4)} over {binary_truth_has0.shape[0]}/{binary_truth_non0.shape[0]}\n"\
                    f"Accuracy all/non0: {np.round(acc_2, 4)}/{np.round(acc_2_non0, 4)}\n"
                    
    return {'mae': mae, 'corr': corr, 'mult': mult_a7, 'f1': f_score, 'acc2': acc_2, 'logger_info':logger_info}

...
class SubNet(nn.Module):
    ...
    def __init__(self, in_size, hidden_size, n_class, dropout, modal_name='text'):
        ...
        self.linear_3 = nn.Linear(hidden_size, 64)  # 128    1
        self.linear_4 = nn.Linear(64, 32)  # 128    1
        self.linear_5 = nn.Linear(32, 16)  # 128    1
        self.linear_6 = nn.Linear(16, 8)  # 128    1
        self.linear_7 = nn.Linear(8, n_class)
    ...
    def forward(self, x):
        ...
        y_5 = self.linear_5(y_4)
        y_6 = self.linear_6(y_5)
        y_7 = self.linear_7(y_6)
        return [y_3,y_4,y_5,y_6], y_7
    
class conv_SubNet(nn.Module):
    def __init__(self, in1, in2, out):
        super().__init__()
        self.linear_1 = nn.Linear(in1, in1)  # 192   128
        self.linear_2 = nn.Linear(in2, in2)
        self.conv = nn.Conv1d(
            in_channels=in1+in2, out_channels=out, kernel_size=1)
        # self.conv_lv = nn.Conv1d(
        #     in_channels=832, out_channels=64, kernel_size=1)
        # self.conv_va = nn.Conv1d(
        #     in_channels=128, out_channels=64, kernel_size=1)
        # self.conv_la = nn.Conv1d(
        #     in_channels=832, out_channels=64, kernel_size=1)

    def forward(self, x1, x2):
        y_1 = torch.tanh(self.linear_1(x1))
        y_2 = torch.tanh(self.linear_2(x2))
        y_3 = self.conv(torch.cat([y_1, y_2], dim=1).unsqueeze(2)).squeeze()
        return y_3
...
def get_resNet4(input_dim = 896):
    b1 = nn.Sequential(nn.Conv1d(input_dim, 512, kernel_size=1, stride=1),
                       nn.ReLU())
    net = nn.Sequential(b1, )
    return net

def get_resNet5(input_dim = 192):
    b1 = nn.Sequential(nn.Conv1d(input_dim, 512, kernel_size=1, stride=1),
                       nn.ReLU())
    net = nn.Sequential(b1, )
    return net
